...
func TestWireGuardTCPBaseline(t *testing.T) {
	...
	hubPriv, hubPub, err := tunnel.GenerateKeyPair()
	if err != nil {
		t.Fatal(err)
	}
	...
	hubPrivHex, _ := tunnel.KeyToHex(hubPriv)
	hubPubHex, _ := tunnel.KeyToHex(hubPub)
	peerPrivHex, _ := tunnel.KeyToHex(peerPriv)
	peerPubHex, _ := tunnel.KeyToHex(peerPub)

	hubTun, hubNet, err := wgnetstack.CreateNetTUN(
	...
	clientTun, clientNet, err := wgnetstack.CreateNetTUN(
...
func TestWireGuardTCPWithFilterAndForwarding(t *testing.T) {
	...
	hubMgr, err := tunnel.NewManager("10.8.0.1", "10.8.0.1", nil, hubPort, 1420)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := hubMgr.SyncPeer(runtime.WGPeer{PublicKey: peerPub, WGIP: "10.8.0.2", Enabled: true}); err != nil {
		t.Fatal(err)
	}

	if err := vpnnetstack.EnableForwarding(hubMgr.Net()); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := ingress.StartWebServer(hubMgr.Net(), "10.8.0.1", 18082, mux); err != nil {
		t.Fatal(err)
	}
...
func TestWireGuardTCPWithEnableForwardingOnly(t *testing.T) {
	...
	if err := vpnnetstack.EnableForwarding(hubNet); err != nil {
		t.Fatal(err)
	}
	hubDev := device.NewDevice(hubTun, conn.NewDefaultBind(), device.NewLogger(device.LogLevelError, ""))
	if err := hubDev.IpcSet(fmt.Sprintf("private_key=%s\nlisten_port=%d\n", hubPrivHex, hubPort)); err != nil {
