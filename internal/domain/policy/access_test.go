...
// peerCanSend mirrors block-list semantics from AccessPolicySpec.
func peerCanSend(spec AccessPolicySpec, from, to string) bool {
	fromA, toA := addr(from), addr(to)
	for _, b := range spec.Blocked[fromA] {
		if b == toA {
			return false
		}
	}
...
// peersCanReach is true when both peers can send to each other.
func peersCanReach(spec AccessPolicySpec, from, to string) bool {
	return peerCanSend(spec, from, to) && peerCanSend(spec, to, from)
}

func buildAccessSpec(peers []PeerEndpoint, links []GroupLinkPair, groups GroupAccessPolicy, maps MapAccessPolicy) (AccessPolicySpec, error) {
	return BuildAccessPolicySpec(peers, links, groups, maps)
}
...
func TestBuildAccessRules_SameGroupInterconnect(t *testing.T) {
	...
	spec, err := buildAccessSpec(peers, nil, GroupAccessPolicy{}, MapAccessPolicy{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildAccessRules_SameGroupIntraDisabled(t *testing.T) {
	...
	spec, err := buildAccessSpec(peers, nil, grp, MapAccessPolicy{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if peerCanSend(spec, "100.127.0.2", "100.127.0.4") || peerCanSend(spec, "100.127.0.4", "100.127.0.2") {
...
func TestBuildAccessRules_MixedIntraGroupSettings(t *testing.T) {
	...
	if peersCanReach(spec, "100.127.0.2", "100.127.0.3") {
		t.Fatal("group 10 intra disabled")
	}
	if !peersCanReach(spec, "100.127.0.4", "100.127.0.5") {
...
func TestBuildAccessRules_LinkedGroupsInterconnect(t *testing.T) {
	...
	spec, err := buildAccessSpec(peers, links, GroupAccessPolicy{}, MapAccessPolicy{})
	if err != nil {
		t.Fatal(err)
	}

	if !peersCanReach(spec, "100.127.0.3", "100.127.0.4") {
		t.Fatal("linked groups 2 and 3 should reach each other")
	}
	if peersCanReach(spec, "100.127.0.2", "100.127.0.3") {
		t.Fatal("group 1 must stay isolated from group 2 without a link")
	}
	if peersCanReach(spec, "100.127.0.2", "100.127.0.4") {
...
func TestBuildAccessRules_LinkedGroupsUnaffectedByIntraDisabled(t *testing.T) {
	...
	spec, err := buildAccessSpec(peers, links, grp, MapAccessPolicy{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildAccessRules_DisabledPeerSkipped(t *testing.T) {
	...
	if !peerCanSend(spec, "100.127.0.2", "100.127.0.3") {
		t.Fatal("enabled peer should not block disabled peer destination")
	}
	if !peerCanSend(spec, "100.127.0.3", "100.127.0.2") {
...
func TestBuildAccessRules_NoGroupPeerSkipped(t *testing.T) {
	...
	if _, ok := spec.Blocked[addr("100.127.0.3")]; ok {
...
func TestBuildAccessPolicySpec(t *testing.T) {
	peers := []PeerEndpoint{
		{ID: 1, WGIP: "100.127.0.2", GroupID: 1, Enabled: true},
		{ID: 2, WGIP: "100.127.0.3", GroupID: 2, Enabled: true},
		{ID: 3, WGIP: "100.127.0.4", GroupID: 3, Enabled: true},
...
func TestBuildAccessRules_MapGroupACL(t *testing.T) {
	...
	spec, err := buildAccessSpec(peers, nil, GroupAccessPolicy{}, maps)
	if err != nil {
		t.Fatal(err)
	}
	if !peerCanSend(spec, "100.127.0.2", "100.127.0.50") {
		t.Fatal("group 10 should reach map vip")
	}
	if peerCanSend(spec, "100.127.0.3", "100.127.0.50") {
...
func TestBuildAccessRules_UnidirectionalSNAT_IntraDisabledDoesNotBlockCrossGroup(t *testing.T) {
	...
	if !peerCanSend(spec, "100.127.0.2", "100.127.0.4") {
		t.Fatal("uni link SNAT path must remain open")
	}
	if peerCanSend(spec, "100.127.0.4", "100.127.0.2") {
