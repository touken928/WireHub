...
func RegisterRoutes(r *gin.Engine, svc *Server, authSvc *auth.Service) {
	...
	api := r.Group("/api")
	{
		...
		// WebSocket auth uses ?token= (browsers cannot set Authorization on upgrade).
		api.GET("/ws/status", svc.handleStatusWS)
		...
		protected.Use(auth.Middleware(authSvc))
		{
			...
			protected.GET("/maps", svc.handleListMaps)
			protected.POST("/maps", svc.handleCreateMap)
			protected.PUT("/maps/:id", svc.handleUpdateMap)
			protected.DELETE("/maps/:id", svc.handleDeleteMap)
