...
func NewManager(hubIP string, dnsIP string, listenPort int, mtu int) (*Manager, error) {
	...
	filterTUN := network.NewFilterTUN(safeTUN, hub)
...
func (m *Manager) SetAccessRules(rules *network.AccessRuleSet) {
...
func (m *Manager) SyncPeer(peer *store.Peer) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) SyncAll(peers []store.Peer) error {
	for _, p := range peers {
