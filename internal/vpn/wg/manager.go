...
func NewManager(hubIP string, dnsIP string, listenPort int, mtu int) (*Manager, error) {
	...
	filterTUN := filter.NewTUN(safeTUN, hub)
...
func (m *Manager) SetAccessRules(rules *filter.RuleSet) {
...
func (m *Manager) SyncPeer(peer *repo.Peer) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) SyncAll(peers []repo.Peer) error {
	for _, p := range peers {
