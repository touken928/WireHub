...
function buildPath(
  ...
  return samples
    .map((sample, index) => {
      const x = PAD.left + ((sample.t - windowStart) / WINDOW_MS) * innerW;
      const y = PAD.top + innerH - (maxY > 0 ? (sample[key] / maxY) * innerH : innerH);
...
function NetworkUsageChart() {
  ...
  const recordSample = useCallback((peerList: PeerStatus[]) => {
    ...
    if (prev && tick > prev.t) {
      ...
      setSamples((current) => [...current, sample].filter((point) => point.t >= cutoff));
    }
  ...
  const visible = useMemo(
    () => samples.filter((sample) => sample.t >= windowStart),
    [samples, windowStart],
  );

  const maxY = useMemo(
    () => Math.max(1024, ...visible.flatMap((sample) => [sample.rxBps, sample.txBps])),
    [visible],
  );
    ...
    <Card className={styles.card}>
      ...
      <div ref={wrapRef} className={styles.chartWrap}>
        <svg className={styles.svg} viewBox={`0 0 ${width} ${CHART_HEIGHT}`} preserveAspectRatio="none">
          {yTicks.map((value) => {
            const y = PAD.top + innerH - (maxY > 0 ? (value / maxY) * innerH : innerH);
            return (
              <g key={value}>
